...
var toCurrency = function(x, symbol = "USD") {
  ...
  const workableString =
    noLeadingZeros.length < 3
      ? `${"000".substr(
          0,
          3 - noLeadingZeros.replace(/^[0]+/g, "").length
        )}${noLeadingZeros}`
      : noLeadingZeros;
  const stringWithDecimal = `${workableString.substr(
    0,
    workableString.length - 2
  )}.${workableString.substr(workableString.length - 2)}`;
